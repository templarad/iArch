...
public class UncertainMethodViewPart extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		for (int i = 0; i < 4; i++) {
			TreeItem item = new TreeItem(tree, SWT.TOGGLE);
			item.setText(new String[] { "ClassName " + i, "-", "-" });
			item.setImage(JavaUI.getSharedImages().getImage(ISharedImages.IMG_OBJS_CLASS));
			for (int j = 0; j < 4; j++) {
				TreeItem subItem = new TreeItem(item, SWT.BORDER);
				subItem.setText(new String[] { "MethodName " + j, "Alternative", "-" });
				subItem.setImage(JavaUI.getSharedImages().getImage(ISharedImages.IMG_FIELD_PRIVATE));
			}
		}
	}
